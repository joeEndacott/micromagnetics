...
impl VectorField1D {
    ...
    pub fn vector_field_to_scalar_fields(
        ...
        Ok([
            ScalarField1D::new_scalar_field(&self.grid, &vx_values).unwrap(),
            ScalarField1D::new_scalar_field(&self.grid, &vy_values).unwrap(),
            ScalarField1D::new_scalar_field(&self.grid, &vz_values).unwrap(),
        ])
    }
...
mod tests {
    ...
    mod vector_calculus_operations {
        ...
        fn test_partial_x() {
            ...
            assert!(
                partial_x_vector_field.test_equality(&expected_result, 1e-6)
            );
        ...
        fn test_laplacian() {
            ...
            assert!(
                laplacian_vector_field.test_equality(&expected_result, 1e-6)
            );
            ...
            assert!(
                laplacian_vector_field.test_equality(&expected_result, 1e-2)
            );
        }
        ...
        fn test_divergence() {
            ...
            assert!(
                divergence_scalar_field.test_equality(&expected_result, 1e-6)
            );
