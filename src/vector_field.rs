...
///
/// The vector field can be subject to boundary conditions, which are specified
...
impl VectorField1D {
    ...
    /// point and specified boundary conditions (BCs). If the field values do
    /// not match the BCs, the field values are modified to satisfy the BCs.
    ///
    pub fn new_vector_field(
        ...
        let mut vector_field = VectorField1D {
            grid: grid.clone(),
            field_values: field_values.clone(),
            boundary_conditions: boundary_conditions.clone(),
        };

        vector_field.apply_boundary_conditions()?;
    ...
    ///
    pub fn function_to_vector_field<F>(
        grid: &Grid,
        func: F,
        boundary_conditions: &BoundaryConditions,
    ) -> Result<Self, &'static str>
    ...
    ///
    pub fn vector_field_to_scalar_fields(self: &Self) -> [ScalarField1D; 3] {
...
impl VectorField1D {
    ...
    /// vector field.
    ///
    /// The boundary conditions of the new vector field must be specified.
    ///
    pub fn add(
        self: &Self,
...
impl VectorField1D {
    ...
    ///
    /// ## TODO
    /// Add error handling for BCs once BCs are implemented for ScalarField1D.
    ///
    pub fn dot_product(
        self: &Self,
        vector_field: &VectorField1D,
    ...
    mod vector_field_1d_tests {
        ...
        mod constructor_functions_tests {
            ...
            fn test_new_vector_field() {
                ...
                let vector_field = VectorField1D::new_vector_field(
                    &grid,
                    &field_values,
            ...
            fn test_function_to_vector_field() {
                let grid = Grid::new_uniform_grid(0.0, 1.0, 11);
                let vector_field = VectorField1D::function_to_vector_field(
                    &grid,
                    |x| [x, x.powi(2), x.powi(3)],
                    &BoundaryConditions::None,
                );
        ...
        mod arithmetic_operations_tests {
            ...
            fn test_add_vector_fields() {
                ...
                let vector_field_2 =
                    VectorField1D::new_vector_field(&grid, &field_values_2)
                        .unwrap();
                let vector_field_sum =
                    vector_field_1.add(&vector_field_2).unwrap();
                let expected_field_values =
                    vec![[1.0, 1.0, 0.0]; grid.grid_points.len()];
                assert_eq!(
                    vector_field_sum.field_values,
            ...
            fn test_scale_vector_field() {
                ...
                assert_eq!(
                    scaled_vector_field.field_values,
                    expected_field_values
                );
            }
        ...
        mod vector_calculus_tests {
            ...
            fn test_partial_x() {
                ...
                assert!(partial_x_vector_field
                    .test_equality(&expected_result, 1e-6));
            ...
            fn test_laplacian() {
                ...
                assert!(laplacian_vector_field
                    .test_equality(&expected_result, 1e-6));

                // Test the Laplacian of the vector field [x^2, -2x^2, 4x^2].
                let vx =
                    ScalarField1D::function_to_scalar_field(&grid, |x| x * x);
                let vy = ScalarField1D::function_to_scalar_field(&grid, |x| {
                    -2.0 * x * x
                });
                let vz = ScalarField1D::function_to_scalar_field(&grid, |x| {
                    4.0 * x * x
                });
                let vector_field =
                    VectorField1D::scalar_fields_to_vector_field((
                ...
                assert!(laplacian_vector_field
                    .test_equality(&expected_result, 1e-2));
            }
            ...
            fn test_divergence() {
                ...
                assert!(divergence_scalar_field
                    .test_equality(&expected_result, 1e-6));
            ...
            fn test_curl() {
                ...
                let vx = ScalarField1D::function_to_scalar_field(&grid, |x| {
                    x.powi(2)
                });
                let vy = ScalarField1D::function_to_scalar_field(&grid, |x| x);
                let vz = ScalarField1D::function_to_scalar_field(&grid, |x| -x);
