use crate::{
    ...
    errors::{ArithmeticError, FieldError},
    grid::Grid1D,
    // quadratic_interpolation,
    traits::FieldArithmetic,
    utils,
};
...
impl Field for ScalarField1D {
    type Grid = Grid1D;
    type FieldValues = Vec<f64>;
    type BoundaryConditions = BoundaryConditions1D;
...
impl ScalarField1D {
    ...
    ) -> Result<ScalarField1D, FieldError> {
        // Handles grid mismatch error.
...
impl FieldArithmetic for ScalarField1D {
    /// # Add scalar fields
    ///
    /// ## Description
    ...
    ///
    fn add(&self, scalar_field: &ScalarField1D) -> Result<Self, FieldError> {
    ...
    /// The new scalar field is given the default boundary conditions.
    ///
    fn divide(&self, scalar_field: &ScalarField1D) -> Result<Self, FieldError> {
        if self.grid != scalar_field.grid {
            return Err(FieldError::GridMismatch);
        }
        ...
        let field_values: Vec<f64> = self
            ...
            .map(|(v1, v2)| {
                if *v2 == 0.0 {
                    return Err(FieldError::ArithmeticError(
                        ArithmeticError::DivisionByZero,
                    ));
                }
                Ok(v1 / v2)
            })
            .collect::<Result<Vec<f64>, FieldError>>()?;
    ...
    /// The new scalar field is given the default boundary conditions.
    ///
    fn scale(&self, scalar: f64) -> Result<Self, FieldError> {
...
impl ScalarField1D {
    ...
    pub fn apply_scalar_bcs(
        ...
        let num_points = self.field_values.len();
        let mut scalar_field = self.clone();

        match boundary_conditions {
            BoundaryConditions1D::None => {
                scalar_field.boundary_conditions = BoundaryConditions1D::None;
                Ok(scalar_field)
            }
            BoundaryConditions1D::Periodic => {
...
mod tests {
    ...
    mod constructor_functions {
        ...
        fn test_new_scalar_field_error() {
            ...
            assert!(matches!(result.err().unwrap(), FieldError::GridMismatch));
        }
    ...
    mod arithmetic_operations {
        ...
        fn test_scale() {
            ...
            let scalar_field =
                ScalarField1D::new_scalar_field(&grid, &field_values).unwrap();
            let result = scalar_field.scale(2.0).unwrap();
    ...
    mod boundary_conditions {
        ...
        fn test_apply_scalar_bcs_none() {
            ...
            let scalar_field =
                ScalarField1D::new_scalar_field(&grid, &field_values).unwrap();
            let boundary_conditions = BoundaryConditions1D::None;
            let result = scalar_field.apply_scalar_bcs(&boundary_conditions);
            assert!(result.is_ok());
            let updated_scalar_field = result.unwrap();
            assert_eq!(
                updated_scalar_field.boundary_conditions,
                BoundaryConditions1D::None
            );
        }
        ...
        fn test_apply_scalar_bcs_periodic() {
            ...
            assert_eq!(
                updated_scalar_field.field_values[0],
                updated_scalar_field.field_values[4]
            );
        }
        ...
        fn test_apply_scalar_bcs_dirichlet() {
            ...
            assert_eq!(updated_scalar_field.field_values[0], 1.0);
            assert_eq!(updated_scalar_field.field_values[4], 5.0);
        }
        ...
        fn test_apply_scalar_bcs_vector_error() {
            ...
            let scalar_field =
                ScalarField1D::new_scalar_field(&grid, &field_values).unwrap();
            let scalar_field_clone = scalar_field.clone();
